...
function App(){
  ...
  const getActiveHabits=(monthKey)=>{
    if(!monthlyHabits[monthKey])return habits;
    ...
    try{
      ...
      try{
        ...
        if(allFiles&&allFiles.length>7){
          const toDelete=allFiles.slice(7);
  ...
  const generateMonthlyReport=useCallback(async(year,month)=>{
    ...
    try{
      ...
      const qtCount=qtKeys.length>0?qtKeys.length+1:1;
  ...
  if(showOnboarding)return<OnboardingScreen onDone={async()=>{await sv('pv-onboarding-seen',true);setShowOnboarding(false);}}/>;  
...
function AdHocPanel({ds,adHocTasks,setAdHocTasksForDay}){
  ...
  const scheduleTaskNotif=async(task,dateStr)=>{
    ...
    try{
      ...
      const now=new Date();
      if(notifDate.getTime()<=now.getTime()+5000)return null;
